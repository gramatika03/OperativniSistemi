...
private:
    ...
    static void handleSoftwareInterrrupt();
...
inline uint64 Riscv::r_a0() {
    uint64 param;
    __asm__ volatile ("mv %[param], a0": [param]"=r"(param));
    return param;
}

inline uint64 Riscv::r_a1() {
    uint64 param;
    __asm__ volatile ("mv %[param], a1": [param]"=r"(param));
    return param;
}

inline uint64 Riscv::r_a2() {
    uint64 param;
    __asm__ volatile ("mv %[param], a2": [param]"=r"(param));
    return param;
}

inline uint64 Riscv::r_a3() {
    uint64 param;
    //__asm__ volatile ("mv %[param], a3": [param]"=r"(param));
    __asm__ volatile("ld %0, 104(s0)": "=r"(param));
    return param;
}

inline uint64 Riscv::r_a4() {
    uint64 param;
    __asm__ volatile ("ld %[param], 112(s0)": [param]"=r"(param));
    return param;
}

inline void Riscv::w_a0(uint64 val) {
    __asm__ volatile ("sd %0, 80(s0)" : :"r"(val));
}

inline void Riscv::w_a1(uint64 val) {
    __asm__ volatile ("mv a1, %[param]" : : [param]"r"(val));
}
